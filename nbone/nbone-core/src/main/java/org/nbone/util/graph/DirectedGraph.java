...
/**
 * 引用xwork2有向图数据结构 ,原始类为final class,此处为方便扩展取消final
 * @author thinking
 * @since 2016-09-23
 *
 * @param <T>
 * @see  com.opensymphony.xwork2.config.providers.DirectedGraph
 */
public  class DirectedGraph<T> implements Iterable<T> {
    ...
    public void addEdge(T start, T dest) {
        ...
        if (!mGraph.containsKey(start)) {
            throw new NoSuchElementException("The start node does not exist in the graph." + start);
        } else if (!mGraph.containsKey(dest)) {
            throw new NoSuchElementException("The destination node does not exist in the graph." + dest);
        }
    ...
    public boolean edgeExists(T start, T end) {
        ...
        if (!mGraph.containsKey(start)) {
            ...
            throw new NoSuchElementException("The end node does not exist in the graph." + end);
        }
